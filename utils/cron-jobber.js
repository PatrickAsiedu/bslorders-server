...
const getOrders = async (menu_date) => {
  ...
  try {
    ...
    // console.log(menu_id.rows[0].id)
    const result = await pool.query(
      'SELECT id, user_id, food_id, drink_id, comment FROM orders WHERE menu_id = $1',
      [menu_id.rows[0].id]
    )
    if (result.rows.length === 0) {
      return console.log('No orders yet')
    }
    // get food name and user name
    const orders = []
    let no = 1
    for (const bb of result.rows) {
      const { id, user_id, food_id, drink_id, comment } = bb
      const userName = await pool.query(
        'SELECT name FROM users WHERE id = $1',
        [user_id]
      )
      const foodName = await pool.query('SELECT name FROM food WHERE id = $1', [
        food_id,
      ])
      ...
      // console.log(foodName.rows[0].name, drinkName.rows[0].name)
      const new_item = {
        id: no,
        name: userName.rows[0].name,
        food: foodName.rows[0].name,
        drink: drinkName.rows[0].name,
        comment,
      }
